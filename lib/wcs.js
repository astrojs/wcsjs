(function() {
  ...
  WCS.Math.toRightTriangular = function(mat) {
    var els, i, j, k, kp, multiplier, n, np, p, x, y, _ref, _ref2, _ref3, _ref4;
    n = mat.length;
    k = n;
    kp = mat[0].length;
    for (x = n; n <= 1 ? x <= 1 : x >= 1; n <= 1 ? x++ : x--) {
      i = k - n;
      if (mat[i][i] === 0) {
        for (j = _ref = i + 1, _ref2 = k - 1; _ref <= _ref2 ? j <= _ref2 : j >= _ref2; _ref <= _ref2 ? j++ : j--) {
          if (mat[j][i] !== 0) {
            els = [];
            np = kp;
            for (y = np; np <= 1 ? y <= 1 : y >= 1; np <= 1 ? y++ : y--) {
              p = kp - np;
  ...
  Mapper = (function() {
    ...
    Mapper.prototype.verifyHeader = function(json) {
      ...
      for (axis = 1; 1 <= naxis ? axis <= naxis : axis >= naxis; 1 <= naxis ? axis++ : axis--) {
        ...
        key = 'CDELT' + axis;
        this.wcsobj.cdelt.push(json[key] || 1);
      }
      this.wcsobj.lonpole = json.LONPOLE || 0;
      this.wcsobj.latpole = json.LATPOLE || 0;
      this.wcsobj.equinox = json.EQUINOX || 2000;
      date = new Date();
      this.wcsobj.date_obs = json.DATE_OBS || (date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate());
      this.wcsobj.pc = this.checkCard(json, 'PC', naxis) || this.derive_pc(json);
      this.wcsobj.pc_inv = WCS.Math.matrixInverse(this.wcsobj.pc);
      this.wcsobj.cd = this.check_card(json, 'CD', naxis);
      if (this.wcsobj.cd) {
    ...
    */
    ...
    Mapper.prototype.setProjection = function(json) {
      var conic, cylindrical, polyConic, projection, quadCube, zenithal;
      ...
      projection = this.wcsobj.ctype[0].slice(5);
      if (__indexOf.call(zenithal, projection) >= 0) {
        this.wcsobj.phi_0 = 0;
        this.wcsobj.theta_0 = 90;
        this.wcsobj.alpha_p = self.wcsobj.crval[0];
        return this.wcsobj.delta_p = self.wcsobj.crval[1];
      }
    };
